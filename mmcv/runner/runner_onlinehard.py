    ...
    def train(self, data_loader, **kwargs):
        ...
        for i, data_batch in enumerate(data_loader):
            ...
            if len(result1box)!=0 and len(result2box)!=0:
                ...
                #筛选置信度低于阈值的
                index1=torch.where(result1box[:,8]>0.0)[0].to('cpu').numpy()
                index2=torch.where(result2box[:,8]>0.0)[0].to('cpu').numpy()
            ...
            if len(result1label)!=0 and len(result2label)!=0:
                ...
                with torch.no_grad():
                    ...
                    '''# _c = F.layer_norm(gt_rbbox_feats,(1024,))
                    # print(nn.LayerNorm(1024,elementwise_affine=False)(gt_rbbox_feats))
                    _c = F.normalize(gt_rbbox_feats, p=2, dim=-1)#n,1024
                    prototypes = F.normalize(class_dictionary, p=2, dim=-1)
                    masks = torch.einsum('nd,kmd->nmk', _c, prototypes)#n,m,k
                    out_seg = torch.amax(masks, dim=1)#n,k
                    # out_seg = F.layer_norm(out_seg,(15,))
                    print(gt_labels)
                    # print(out_seg)
                    print(torch.max(out_seg, 1)[1])
                    print(torch.max(out_seg, 1)[0])
                    pdb.set_trace()'''

                    # _c = F.layer_norm(pseudo_rbbox_feats,(1024,))
                    _c = F.normalize(pseudo_rbbox_feats, p=2, dim=-1)#n,1024
                    prototypes = F.normalize(class_dictionary, p=2, dim=-1)
                    masks = torch.einsum('nd,kmd->nmk', _c, prototypes)#n,m,k
                    out_seg = torch.amax(masks, dim=1)#n,k
                    # out_seg1 = torch.mean(masks, dim=1)#n,k
                    # # out_seg = F.layer_norm(out_seg,(15,))
                    # print(pseudo_labels)
                    # print(result1box[:,8])
                    # print(result2box[:,8])
                    ...
                    # cosinescore1=out_seg1[:len(pseudo_labels[0])].gather(1,pseudo_labelss0.to('cuda').reshape(-1,1))
                    # cosinescore2=out_seg1[len(pseudo_labels[0]):].gather(1,pseudo_labelss1.to('cuda').reshape(-1,1))
                    # print(cosinescore1)
                    # print(cosinescore2)

                    cosinescore1=out_seg[:len(pseudo_labels[0])].gather(1,pseudo_labelss0.to('cuda').reshape(-1,1))
                    cosinescore2=out_seg[len(pseudo_labels[0]):].gather(1,pseudo_labelss1.to('cuda').reshape(-1,1))
                    # print(cosinescore1)
                    # print(cosinescore2)

                    index1=torch.where(cosinescore1>0.0)[0].to('cpu').numpy()
                    index2=torch.where(cosinescore2>0.0)[0].to('cpu').numpy()
                    # pdb.set_trace()
