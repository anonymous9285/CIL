...
class Runner(object):
    ...
    def train(self, data_loader, **kwargs):
        ...
        for i, data_batch in enumerate(data_loader):
            ...
            # #筛选一定数量的伪标签
            # if len(result1label)>100:#随机sample
            #     # N,C = result1box.shape
            #     # S = int(NUM_PSESTATE/2)
            #     # pdb.set_trace()
            #     # index = torch.LongTensor(random.sample(range(N), S))
            #     _,index = result1box[:,-1].topk(100,dim=0, largest=True, sorted=False)
            ...
            # if len(result2label)>100:#随机sample
            #     # N,C = result2box.shape
            #     # S = int(NUM_PSESTATE/2)
            #     # pdb.set_trace()
            #     # index = torch.LongTensor(random.sample(range(N), S))
            #     _,index = result2box[:,-1].topk(100,dim=0, largest=True, sorted=False)
            ...
            if len(result1box)!=0 and len(result2box)!=0:
                ...
                #筛选置信度低于阈值的
                threshold1=classthreshold[result1label].to('cuda')
                threshold2=classthreshold[result2label].to('cuda')
                index1=torch.where(result1box[:,8]>threshold1)[0].to('cpu').numpy()
                index2=torch.where(result2box[:,8]>threshold2)[0].to('cpu').numpy()
            ...
            # #draw gt+pesudo 类别+置信度
            # color_white = (255, 255, 255)  
            # colormap = [(random.randint(0, 1), random.randint(0, 1), random.randint(0, 1)),(random.randint(0, 1), random.randint(0, 1), random.randint(0, 1)),(random.randint(0, 1), random.randint(0, 1), random.randint(0, 1)),(random.randint(0, 1), random.randint(0, 1), random.randint(0, 1)),(random.randint(0, 1), random.randint(0, 1), random.randint(0, 1)),(random.randint(0, 1), random.randint(0, 1), random.randint(0, 1)),(random.randint(0, 1), random.randint(0, 1), random.randint(0, 1)),(random.randint(0, 1), random.randint(0, 1), random.randint(0, 1)),(random.randint(0, 1), random.randint(0, 1), random.randint(0, 1)),(random.randint(0, 1), random.randint(0, 1), random.randint(0, 1)),(random.randint(0, 1), random.randint(0, 1), random.randint(0, 1)),(random.randint(0, 1), random.randint(0, 1), random.randint(0, 1)),(random.randint(0, 1), random.randint(0, 1), random.randint(0, 1)),(random.randint(0, 1), random.randint(0, 1), random.randint(0, 1)),(random.randint(0, 1), random.randint(0, 1), random.randint(0, 1))]
            # img=cv2.imread('/workspace/ReDet-master3090/datasets/DOTA_1024/trainval1024/images/{}'.format(data_batch['img_meta'].data[0][0]['name']), cv2.IMREAD_COLOR)
            # if data_batch1['img_meta'][0][0]['flip']:
            #     img = cv2.flip(img, 1)
            ...
            #     j=j.item()
            #     color = (0,0,0)
            #     cv2.line(img, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[1])), color=color,thickness=2, lineType=cv2.LINE_AA)
            #     cv2.line(img, (int(bbox[2]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), color=color, thickness=2, lineType=cv2.LINE_AA)
            #     cv2.line(img, (int(bbox[2]), int(bbox[3])), (int(bbox[0]), int(bbox[3])), color=color,thickness=2, lineType=cv2.LINE_AA)
            #     cv2.line(img, (int(bbox[0]), int(bbox[3])), (int(bbox[0]), int(bbox[1])), color=color, thickness=2, lineType=cv2.LINE_AA)
            #     cv2.putText(img, '%s' % (wordname_15[j]+'_'+str(ii)), (int(bbox[0]), int(bbox[1]) + 10),color=(135,206,250), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=0.5)    
            
            # # 伪标签斜框
            # # for ii in range(len(result1label)):
            # #     bbox=result1box[ii]
            # #     j = result1label[ii]-1
            # #     j=j.item()
            # #     color = (127,255,212)
            # #     for k in range(3):
            # #         cv2.line(img, (int(bbox[k * 2]), int(bbox[k * 2 + 1])), (int(bbox[(k + 1) * 2]), int(bbox[(k + 1) * 2 + 1])), color=color,thickness=2, lineType=cv2.LINE_AA)
            # #     cv2.line(img, (int(bbox[6]), int(bbox[7])), (int(bbox[0]), int(bbox[1])), color=color, thickness=2, lineType=cv2.LINE_AA)
            # #     cv2.putText(img, '%s' % (wordname_15[j]+'_'+str(ii)+'_'+str('%.3f'%result1box[ii,-1].item())), (int(bbox[0]), int(bbox[1]) + 10),color=(135,206,250), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=0.5)
            
            # #伪标签水平框
            # for ii in range(len(result1gt_bboxes)):
            #     bbox=result1gt_bboxes[ii]
            #     j = result1label[ii]-1
            #     j=j.item()
            #     color = (127,255,212)
            #     cv2.line(img, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[1])), color=color,thickness=2, lineType=cv2.LINE_AA)
            ...
            #     cv2.line(img, (int(bbox[0]), int(bbox[3])), (int(bbox[0]), int(bbox[1])), color=color, thickness=2, lineType=cv2.LINE_AA)
            #     cv2.putText(img, '%s' % (wordname_15[j]+'_'+str(ii)+'_'+str('%.3f'%result1box[ii,-1].item())), (int(bbox[0]), int(bbox[1]) + 10),color=(135,206,250), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=0.5)
            # dstpath1 = os.path.join('/workspace/ReDet-master3090/drawgtpse',data_batch['img_meta'].data[0][0]['name'])
            # cv2.imwrite(dstpath1, img)

            # #伪标签mask
            # img_mask=np.zeros((1024,1024))
            # for ii in range(len(result1gt_masks)):
            #     img_mask=result1gt_masks[ii]+img_mask
            # img_mask=img_mask*255
            # dstpath1 = os.path.join('/workspace/ReDet-master3090/drawmask',data_batch['img_meta'].data[0][0]['name'])
            # cv2.imwrite(dstpath1, img_mask)
            

            # img=cv2.imread('/workspace/ReDet-master3090/datasets/DOTA_1024/trainval1024/images/{}'.format(data_batch['img_meta'].data[0][1]['name']), cv2.IMREAD_COLOR)
            # if data_batch2['img_meta'][0][0]['flip']:
            #     img = cv2.flip(img, 1)
            # for ii in range(len(gt_labels[1])):
            #     bbox=gt_bboxes[1][ii]
            #     j = gt_labels[1][ii]-1
            #     j=j.item()
            #     color = (0,0,0)
            ...
            # # 伪标签斜框
            # # for ii in range(len(result2label)):
            # #     bbox=result2box[ii]
            # #     j = result2label[ii]-1
            # #     j=j.item()
            # #     color = (127,255,212)
            ...
            # #         cv2.line(img, (int(bbox[k * 2]), int(bbox[k * 2 + 1])), (int(bbox[(k + 1) * 2]), int(bbox[(k + 1) * 2 + 1])), color=color,thickness=2, lineType=cv2.LINE_AA)
            # #     cv2.line(img, (int(bbox[6]), int(bbox[7])), (int(bbox[0]), int(bbox[1])), color=color, thickness=2, lineType=cv2.LINE_AA)
            # #     cv2.putText(img, '%s' % (wordname_15[j]+'_'+str(ii)+'_'+str('%.3f'%result2box[ii,-1].item())), (int(bbox[0]), int(bbox[1]) + 10),color=(135,206,250), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=0.5)

            # #伪标签水平框
            # for ii in range(len(result2gt_bboxes)):
            #     bbox=result2gt_bboxes[ii]
            #     j = result2label[ii]-1
            #     j=j.item()
            #     color = (127,255,212)
            #     cv2.line(img, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[1])), color=color,thickness=2, lineType=cv2.LINE_AA)
            ...
            # dstpath1 = os.path.join('/workspace/ReDet-master3090/drawgtpse',data_batch['img_meta'].data[0][1]['name'])
            # cv2.imwrite(dstpath1, img)

            # #伪标签mask
            # img_mask=np.zeros((1024,1024))
            # for ii in range(len(result2gt_masks)):
            #     img_mask=result2gt_masks[ii]+img_mask
            # img_mask=img_mask*255
            # dstpath1 = os.path.join('/workspace/ReDet-master3090/drawmask',data_batch['img_meta'].data[0][1]['name'])
            # cv2.imwrite(dstpath1, img_mask)
            ...
            # print(i)
                ...
                #     #筛选特征相似度低于阈值的
                #     # pseudo_bboxes=[torch.from_numpy(result1gt_bboxes),torch.from_numpy(result2gt_bboxes)]#x1,y1,x2,y2 (2,50,)
                ...
                #     class_dictionary = torch.load('class_dictionary_tensor12.pt')
                #     # cosinematrix=torch.sum(torch.cat([pseudo_rbbox_feats]*15,dim=1).reshape(-1,1024)*torch.cat([class_dictionary.to('cuda')]*len(pseudo_rbbox_feats),dim=0),1)/torch.norm(torch.cat([class_dictionary.to('cuda')]*len(pseudo_rbbox_feats),dim=0),p=2,dim=1)/torch.norm(torch.cat([pseudo_rbbox_feats]*15,dim=1).reshape(-1,1024),p=2,dim=1)
                #     # cosinematrix=cosinematrix.reshape(-1,15)
                #     cosinematrix=torch.mm(pseudo_rbbox_feats, class_dictionary.to('cuda').T)/torch.norm(pseudo_rbbox_feats,p=2,dim=1).reshape((-1,1))/torch.norm(class_dictionary.to('cuda'),p=2,dim=1)
                #     similarity=torch.softmax(cosinematrix,dim=1)

                #     # print(similarity[0:len(result1label)]*torch.nn.functional.one_hot(result1label-1, 15).float().to('cuda'))
                #     # print(similarity[len(result1label):]*torch.nn.functional.one_hot(result2label-1, 15).float().to('cuda'))
                    
                #     index1=torch.where(similarity[0:len(result1label)]*torch.nn.functional.one_hot(result1label-1, 15).float().to('cuda')>0.1)[0].to('cpu').numpy()
                #     index2=torch.where(similarity[len(result1label):]*torch.nn.functional.one_hot(result2label-1, 15).float().to('cuda')>0.1)[0].to('cpu').numpy()
            ...
            #         # print("a",a)
            #         # pdb.set_trace()
            #         if torch.nonzero(pseudo_rbbox_feats[a]).numel()==0:
            #             continue
            ...
            #         class_dictionary = torch.load('class_dictionary_tensor12.pt')
            #         cosmatrix = torch.sum(s_pse[a,:1024]*class_dictionary.to('cuda'),1)/torch.norm(s_pse[a,:1024],p=2,dim=0)/torch.norm(class_dictionary.to('cuda'),p=2,dim=1)

            #         r=torch.log(s_pse[a,1024])+ torch.softmax(cosmatrix,dim=0)[slabel-1]
            #         # print(r)
            #         # pdb.set_trace()
            #         if r>REWARD_THRES:
            #             r=1
            ...
            #     # print(np.shape(gt_rbbox_feats))
            #     # pdb.set_trace()
            #     gt_labels=torch.cat((data_batch['gt_labels'].data[0][0],data_batch['gt_labels'].data[0][1]))-1
            #     self.lut[gt_labels,:] = 0.5 * self.lut[gt_labels,:] + (1 - 0.5) * gt_rbbox_feats.to('cpu')
            #     # self.lut[gt_labels,:] /= self.lut[gt_labels,:].norm().reshape((-1,1))
            #     # pdb.set_trace()
            #     # print(self.lut)
            self._iter += 1
        ...
        #保存字典
        # torch.save(self.lut, 'class_dictionary{}.pt'.format(self._epoch)) # 保存
        #保存dqn模型
