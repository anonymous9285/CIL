    ...
    def train(self, data_loader, **kwargs):
        ...
        for i, data_batch in enumerate(data_loader):      
            ...
            if len(result1box)!=0 and len(result2box)!=0:
                ...
                #筛选置信度低于阈值的
                index1=torch.where(result1box[:,8]>0.3)[0].to('cpu').numpy()
                index2=torch.where(result2box[:,8]>0.3)[0].to('cpu').numpy()
                ...
                # result1gt_bboxes=torch.index_select(torch.from_numpy(result1gt_bboxes), 0, torch.LongTensor(index1))
                ...
                # result2gt_bboxes=torch.index_select(torch.from_numpy(result2gt_bboxes), 0, torch.LongTensor(index2))
            ...
            #         self.lut[gt_labels_a,:] = 0.5 * self.lut[gt_labels_a,:] + (1 - 0.5) * gt_rbbox_feats_lst.reshape(-1,1024) 
            self._iter += 1
