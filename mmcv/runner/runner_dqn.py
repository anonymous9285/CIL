...
N_STATES = NUM_GTSTATE*143+NUM_PSESTATE*143  
...
class Net(nn.Module):
    ...
    def forward(self, x):    
        ...
        x = torch.cat((x_pre2,x[:,:,1024:].reshape((len(x),-1,15))),dim=2)
...
class DQN(object):
    def __init__(self):                                                         # 定义DQN的一系列属性
        ...
        self.eval_net.load_state_dict(torch.load("/workspace/ReDet-master3090/dqn_model11.pkl"))
        self.target_net.load_state_dict(torch.load("/workspace/ReDet-master3090/dqn_model11.pkl"))
        self.learn_step_counter = 0                                             # for target updating
        self.memory_counter = 0                                                 # for storing memory
        self.memory = [{'s':torch.zeros((NUM_GTSTATE+NUM_PSESTATE,1024+15)),'a':0,'r':0,'s_':torch.zeros((NUM_GTSTATE+NUM_PSESTATE,1024+15))}]*MEMORY_CAPACITY             # 初始化记忆库，一行代表一个transition
...
class Runner(object):
    ...
    def train(self, data_loader, **kwargs):
        ...
        for i, data_batch in enumerate(data_loader):
            ...
            fullname = os.path.join('/workspace/ReDet-master3090/datasets/DOTA_1024/trainval1024/pselabelTxt_0.05', data_batch['img_meta'].data[0][0]['name'].strip('.png') + '.txt')
            f = open(fullname, 'r')
            ...
            fullname = os.path.join('/workspace/ReDet-master3090/datasets/DOTA_1024/trainval1024/pselabelTxt_0.05', data_batch['img_meta'].data[0][1]['name'].strip('.png') + '.txt')
            f = open(fullname, 'r')
            for line in f.readlines():
                splitlines = line.strip().split(' ')
            ...
            if len(result1label)!=0 and len(result2label)!=0:
                ...
                with torch.no_grad():
                    ...
                    # pdb.set_trace()
                    probility = torch.nn.functional.one_hot(torch.cat((gt_labels[0],gt_labels[1]))-1, 15).float().to('cuda')
                    gt_rbbox_feats=torch.cat((gt_rbbox_feats,probility),dim=1)
                    gt_rbbox_feats=torch.cat((gt_rbbox_feats,torch.zeros((NUM_GTSTATE-len(gt_rbbox_feats),1024+15)).to('cuda')))#torch.Size([40, 1025])
                    ...
                    # pdb.set_trace()
                    probility = torch.nn.functional.one_hot(torch.cat((pseudo_labels[0],pseudo_labels[1]))-1, 15).float().to('cuda')
                    aaaa=probility*torch.cat((result1box[:,8],result2box[:,8])).reshape(-1,1).type(torch.float32)
                    pseudo_rbbox_feats=torch.cat((pseudo_rbbox_feats,aaaa),dim=1) 
                    pseudo_rbbox_feats=torch.cat((pseudo_rbbox_feats,torch.zeros((NUM_PSESTATE-len(pseudo_rbbox_feats),1024+15)).to('cuda'))) #torch.Size([80, 1025])
                ...
                while torch.nonzero(pseudo_rbbox_feats).numel()!=0 and num_gt < NUM_GTSTATE:                 
                    a  = self.dqn.choose_action(torch.cat((gt_rbbox_feats,pseudo_rbbox_feats),dim=0))#torch.Size([120+100, 1024+15])
                    # print("a",a)
                    # pdb.set_trace()
        ...
        torch.save(self.dqn.target_net.state_dict(), "dqn_model2{}.pkl".format(self._epoch))
        self._epoch += 1
