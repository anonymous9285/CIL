...
def DOTA2COCOTrain_cut(srcpath, destfile, cls_names, difficult='2', ext='.png',ratio=0.01):
    ...
    labelparent = os.path.join(srcpath, 'labelTxt_{}'.format(ratio))
...
class splitbase_cut():
    def __init__(self,
        ...
        self.labelpath = os.path.join(self.basepath, 'labelTxt_{}'.format(self.ratio))
...
def parse_args():
    ...
    parser.add_argument('--dstpath', default='datasets/DOTA_1024_ms',
                        help='prepare data')
...
def cut_v2(srcpath, dstpath, ratio):
    split_train = splitbase_cut(os.path.join(srcpath, 'train'),os.path.join(dstpath, 'trainval1024'),
                                gap=500,subsize=1024,num_process=16,ratio=ratio)
    split_train.splitdata(1)

    split_train_ms = splitbase_cut(os.path.join(srcpath, 'train'),os.path.join(dstpath, 'trainval1024_ms'),
                                gap=500,subsize=1024,num_process=16,ratio=ratio)
    ...
    split_val_ms = splitbase_cut(os.path.join(srcpath, 'val'),os.path.join(dstpath, 'trainval1024_ms'),
                                gap=500,subsize=1024,num_process=16,ratio=ratio)
    split_val_ms.splitdata(0.5)
    split_val_ms.splitdata(1.5)

    DOTA2COCOTrain_cut(os.path.join(dstpath, 'trainval1024'), os.path.join(dstpath, 'trainval1024', 'DOTA_trainval1024_{}.json'.format(ratio)), 
                                                                                                                        wordname_15, difficult='2',ratio=ratio)
    DOTA2COCOTrain_cut(os.path.join(dstpath, 'trainval1024_ms'), os.path.join(dstpath, 'trainval1024_ms', 'DOTA_trainval1024_ms_{}.json'.format(ratio)), 
                                                                                                                        wordname_15, difficult='2',ratio=ratio)
